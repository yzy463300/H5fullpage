...
puzzleGame.prototype = {
    ...
    mousemove: function(event) {
		...
		var deltaX = event.originalEvent.targetTouches[0].pageX - this.start.pageX;
		var deltaY = event.originalEvent.targetTouches[0].pageY - this.start.pageY;
		//元素移动的距离
        this.element.css({
    ...
    calculateOverflow:function(event){
        ...
        //左边边界
        console.log('offsetX:   ' + event.target.offsetLeft);
    ...
    calculateOverlap: function(event, startDebrisIndex) {
        ...
        var col = Math.floor((event.originalEvent.changedTouches[0].pageY - this.contentTop) / this.debrisWidth),
            row = Math.floor((event.originalEvent.changedTouches[0].pageX - this.contentLeft) / this.debrisHeight);
        //从整数1开始算起
        col = col + 1; //列
        row = row + 1; //行
        //索引位置
    ...
    creatEvent: function() {
        ...
        var self = this;
        this.$contentArea.on('touchstart',function(event) {
          stopBehavior(event);
          self.mousedown(event);
        });
        this.$contentArea.on('touchmove',function(event) {
          stopBehavior(event);
          self.mousemove(event);
        });
        this.$contentArea.on('touchend',function(event) {
           stopBehavior(event);
           self.mouseup(event);
        });
    },
